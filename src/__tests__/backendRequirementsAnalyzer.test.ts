...
describe('BackendRequirementsAnalyzer', () => {
    ...
    it('should trigger AI fallback when confidence is below threshold', async () => {
      ...
      const mockAI = {
        analyze: vi.fn().mockResolvedValue({
          entities: [{ name: 'User', typeName: 'User', fields: [], confidence: 85, matchType: 'ai' }],
          hasAuth: false,
          authRequirements: [],
          hasStorage: false,
          storageRequirements: [],
  ...
  describe('hybrid orchestration', () => {
    it('should combine pattern and AI results when hybrid mode', async () => {
      const mockAI = {
        analyze: vi.fn().mockResolvedValue({
          entities: [{ name: 'User', typeName: 'User', fields: [{ name: 'email', type: 'string', isOptional: false }], confidence: 85, matchType: 'ai' }],
          hasAuth: true,
