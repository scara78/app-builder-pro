...
describe('Phase 7: Integration Tests', () => {
  ...
  describe('7.3: Operation isolation', () => {
    it('should not affect project when operations fail on invalid ref', async () => {
      ...
      await expect(client.getAnonKey('not-found')).rejects.toThrow(MCPNotFoundError);
    ...
    it('should handle multiple sequential operations on same project', async () => {
      ...
      await client.applyMigration(project.ref, 'CREATE TABLE users (id INT);', 'create-users');

      await client.applyMigration(project.ref, 'CREATE TABLE posts (id INT);', 'create-posts');
