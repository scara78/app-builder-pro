 ...
 */
function transformToBackendRequirements(
  analysis: ReturnType<PatternMatcher['analyze']>
): BackendRequirements {
  return {
...
describe('E2E Integration: React Code → SQL Generation', () => {
  ...
  describe('Scenario 2: Auth + Posts with Relationships', () => {
    it('should generate users and posts tables with FK and RLS', () => {
      ...
      console.log(
        'Scenario 2 - Auth detected:',
        requirements.authRequirements?.map((a) => a.type)
      );
    });
